...
We conducted a rigorous A/B test to validate these optimizations.

### A. RAVE vs UCT (Algorithmic Performance)
*   **Setup**: 20 Games, 50ms/move, Alternating Colors (Fair).
*   **Configuration**: Unoptimized (No TT) to isolate algorithmic impact.
*   **Result**: **RAVE won 16/20 games (80% Win Rate).**
*   **Analysis**: Even without the Transposition Table, RAVE significantly outperforms UCT by learning from "all moves as first".

![Win Rate Graph](plots/win_rate.png)
...
![NPS Graph](plots/nps.png)

#### 2. Search Stability (Consistency)
*   **Observation**: The Optimized agent played **perfectly consistent games** (exactly 35 moves every time), whereas the Unoptimized agent was erratic (27-67 moves).
*   **Conclusion**: The Transposition Table stabilizes the search so effectively that it allows the agent to hold the **First Player Advantage** 100% of the time. In our "Fair" optimized test, Red won 20/20 games, proving that the optimizations bring the agent closer to perfect play.

![Consistency Graph](plots/consistency.png)
